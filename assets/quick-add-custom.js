if (!customElements.get("quick-add-custom-component")) {
  class QuickAddCustomComponent extends HTMLElement {
    ...
    init() {
      ...
      this._originalSizesHTML =
        this.querySelector(".quick-add-custom__sizes")?.innerHTML ?? null;

      const cardGallery = this.closest(".card-gallery");
      if (cardGallery) {
        this._mode = "slideshow";
        this._slideshow =
          cardGallery.querySelector("slideshow-component") ?? null;
        this._slideshowScroller =
          this._slideshow?.querySelector('[ref="scroller"]') ?? null;
        this._originalScrollerHTML = this._slideshowScroller?.innerHTML ?? null;
        this._cardImg = null;
        this._cardLink =
          cardGallery.querySelector('[ref="cardGalleryLink"]') ?? null;
        this._originalHref = this._cardLink?.href ?? null;
      } else {
        this._mode = "simple";
        this._cardImg =
          this.closest(".product-card-simple__media")?.querySelector(
    ...
    initializeSelectedOptions() {
      ...
      if (sizeContainer) {
        const firstSize = sizeContainer.querySelector(
          ".quick-add-custom__size-btn.is-selected",
        );
        if (firstSize) {
          this.selectedOptions[firstSize.dataset.optionIndex] =
            firstSize.dataset.optionValue;
        }
      }
      this.updateVariant();
    }
    ...
    bindEvents() {
      ...
      this.addEventListener("click", (e) => {
        const btn = e.target.closest(".quick-add-custom__size-btn");
        if (btn) this.handleOptionClick(e, btn);
      });
    ...
    handleSiblingColorBtn(btn) {
      ...
      if (colorsContainer) {
        colorsContainer
          .querySelectorAll(".quick-add-custom__color-btn")
          .forEach((b) => b.classList.remove("is-selected"));
      }
      btn.classList.add("is-selected");
    ...
    /**
     * Load sibling color buttons using the shared ColorSiblings service.
     * Falls back to a local predictive search if the service isn't available.
     */
    loadQuickAddSiblings(container) {
      ...
      // Use shared service (color-siblings.js) — single cache, no duplicate requests
      if (window.ColorSiblings) {
        window.ColorSiblings.fetch(styleRef).then((siblings) => {
          if (siblings)
      ...
      // Fallback: if color-siblings.js hasn't loaded yet (shouldn't happen normally)
      const params = [
        ...
        .then((json) => {
          ...
          const siblings = results.filter((p) => {
            if (p.tags && p.tags.length)
              return p.tags.some((t) => t.indexOf(erpPrefix) === 0);
    ...
    renderQuickAddSiblings(container, siblings, currentHandle) {
      ...
      const otherSiblings = siblings.filter(
        (s) => s.handle !== currentHandle && s.available !== false,
      );
      ...
      otherSiblings.forEach((sibling) => {
        const colorName = window.ColorSiblings
          ? window.ColorSiblings.getColorName(sibling)
          : this._getSiblingColorName(sibling);
        ...
        if (imgUrl) {
          const optimizedUrl = imgUrl + (imgUrl.indexOf("?") !== -1 ? "&" : "?") + "width=80";
          btn.style.backgroundImage = "url(" + optimizedUrl + ")";
    ...
    /** Fallback color name extraction when ColorSiblings service is unavailable */
    ...
    restoreOriginal() {
      ...
      const firstSize = this.querySelector(
        ".quick-add-custom__size-btn.is-selected",
      );
      ...
      if (this._cardLink && this._originalHref)
        this._cardLink.href = this._originalHref;
    }
    ...
    rebuildFromProduct(productData) {
      ...
      if (this._cardLink)
        this._cardLink.href = "/products/" + productData.handle;
    }
    ...
    updateCardImage(productData) {
      if (this._mode === "slideshow") {
        ...
        this._cardImg.srcset = [200, 300, 400, 500, 600]
    ...
    _resetSlideshowToFirst() {
      ...
      requestAnimationFrame(() => {
        try {
          this._slideshow.select(0, null, { animate: false });
        } catch (e) {}
      });
    }
    ...
    updateVariant() {
      const matchingVariant = this.variants.find((variant) => {
        return Object.keys(this.selectedOptions).every((index) => {
          return (
            variant.options[parseInt(index)] === this.selectedOptions[index]
          );
        });
      });
    ...
    updateSizeAvailability() {
      ...
      sizeContainer
        .querySelectorAll(".quick-add-custom__size-btn")
        .forEach((btn) => {
